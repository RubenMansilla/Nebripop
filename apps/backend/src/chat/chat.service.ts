...
export class ChatService {
    ...
    async getUserChats(userId: number) {
        ...
        const result = await Promise.all(
            chats.map(async (chat) => {
                ...
                // Buscar mensajes de este chat, que NO haya enviado yo, y que no estén leídos.
                const unreadCount = await this.msgRepo.count({
                    where: {
                        chat: { id: chat.id },       // Pertenece a este chat
                        sender: { id: Not(userId) }, // El remitente NO soy yo (es el otro)
                        isRead: false                // No está leído
                    }
                });
    ...
    async markMessagesAsRead(chatId: number, userId: number) {
        // Actualizar a TRUE todos los mensajes de este chat
        // que NO hayan sido enviados por mí (userId)
        await this.msgRepo.update(
            {
                chat: { id: chatId },
                sender: { id: Not(userId) }, // Solo marco como leídos los que recibí
                isRead: false
            },
            { isRead: true }
        );
